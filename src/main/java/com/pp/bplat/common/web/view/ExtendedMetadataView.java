...
public class ExtendedMetadataView extends AbstractView {
	...
	private String getContentType(String metadataType) {
		String result = null;
		if (Constant.Data.JSON.isMatch(metadataType)) result = Constant.RequestKey.CONTENT_TYPE_JSON.getValue(); 
		else if (Constant.Data.XML.isMatch(metadataType)) result = Constant.RequestKey.CONTENT_TYPE_XML.getValue();
		else result = Constant.RequestKey.CONTENT_TYPE_HTML.getValue();
		return result;
	}
	...
	protected void renderMergedOutputModel( Map<String, Object> model, HttpServletRequest request,HttpServletResponse response ) throws Exception {
		...
		if(data != null){
			...
			while(ie.hasNext()) {
				...
				String jsonStr = ananlyzer.generate(entry.getKey(), entry.getValue());
