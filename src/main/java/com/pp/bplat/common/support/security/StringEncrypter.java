...
public class StringEncrypter {
	...
	private enum Format {
		yyyyMMdd("yyyyMMdd"),SECURITY_KEY("kt_prm_common_key"),SECURITY_IV("kt_prm_common_iv"),UTF8("UTF8"),MD5("MD5"),AES("AES");
	...
	private void init(String paramKey, String iv, boolean isChangeKey) {
		...
		try {
			...
			this.key = new SecretKeySpec(md5.digest(key.getBytes(Format.UTF8.getValue())), Format.AES.getValue());
			this.initalVector = new IvParameterSpec(md5.digest(iv.getBytes(Format.UTF8.getValue())));
		} catch (Exception ex) {
			throw new PartnerException(Error.E0006.getValue());
		}
	}
